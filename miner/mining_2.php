...
$nonceLimit = 10000 - 1;
$nonce = (int)$_GET['start'];
$nonceTill = $nonce + $nonceLimit;

echo "<p>Grab next block</p>";
echo "<p>".$_SESSION['template']. "</p>";
echo "<p>Attempting on nonce from {$nonce} to {$nonceTill}</p>";
ob_flush();
flush();
...
<html>
	<head>
		<meta http-equiv="refresh" content="3;url=?start=<?php echo $startNonce?>" />
	</head>
	<body>
